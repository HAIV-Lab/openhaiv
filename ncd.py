...
image_paths, X, true_labels = load_images_from_txt(
    id_txt_file, ood_txt_file, model, test_transform, batch_size=64
)
...
print(
    f"Remapped cluster distribution: {np.bincount(remapped_labels[remapped_labels != -1])}"
)  # Ignore unmatched labels (-1)
print(f"Normalized Clustering Distance (NCD): {ncd_value:.4f}")
