...
parser.add_argument(
    ...
    nargs="+",
    default=[
        "configs/supervised/sl_remote_alice.yaml",
        "configs/inference/remote.yaml",
    ],
...
def main(args):
    ...
    for session in range(num_session):
        if session == 0:
            img_list, gt_list, pred_num = generate_classify_datasets(
                model_det, cfg, cfg.det_data_0, cfg.xml_data_0
            )
            _, train_loader, test_loader = cli_dataloader(cfg, 0)
            max_cls = max(test_loader.dataset.targets)
            indices_to_remove = [
                index for index, num in enumerate(gt_list) if num > max_cls
            ]
            gt_list = [num for num in gt_list if num <= max_cls]
            img_list = [
                elem
                for index, elem in enumerate(img_list)
            ...
            img_list, gt_list, pred_num = generate_classify_datasets(
                model_det, cfg, cfg.det_data_1, cfg.xml_data_1
            )
