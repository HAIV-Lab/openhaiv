...
from ncdia_old.utils import setup_logger, set_seed
from ncdia_old.quantize import quantize_pack, reconstruct

class Train_SAVC_ATT_New_Pipeline:
    ...
    def run(self):
        ...
        for session in range(1, self.config.CIL.sessions): # start from session 1 
            ...
            val_metrics = evaluator.eval_acc_attr(net, prev_val_loader, None, session-1, 0)
            evaluator.eval_ood(net, {'train': prev_train_loader, 'test': prev_val_loader}, cur_train_loader, session-1)

            # --------- novel class discover -------- #
            if self.config.discoverer.name == 'savc_att_discoverer':
                prev_val_loader.dataset.sample_test_data(ratio=self.config.discoverer.prev_valset_ratio)
                cur_train_loader = discoverer.get_pseudo_newloader(net, {'train': prev_train_loader, 'test': prev_val_loader}, 
                                                cur_train_loader, train_tfm, session - 1)
            elif self.config.discoverer.name == 'savc_sskm_discoverer':
                prev_val_loader.dataset.sample_test_data(shot=self.config.CIL.shot//3)
                # prev_val_loader.dataset.sample_test_data(ratio=self.config.discoverer.prev_valset_ratio)
                cur_train_loader = discoverer.get_pseudo_SemiSupKMeans(net, {'train': prev_train_loader, 'test': prev_val_loader}, 
                                                cur_train_loader, train_tfm, session)
            ...
            # val_metrics = evaluator.eval_acc_attr(net, prev_val_loader, None, session-1, 0)
            val_metrics = evaluator.eval_acc_attr(net, cur_val_loader, None, session, 0)
            # print(val_metrics)
            ...
            if self.config.CIL.incft:
                ...
                val_metrics = evaluator.eval_acc_attr(net, cur_val_loader, None, session, epoch_idx)
                comm.synchronize()
                if comm.is_main_process():
        ...
        if self.config.quantizer.apply:
            test_metrics = evaluator.eval_acc_attr(net, cur_test_loader, None, session)
