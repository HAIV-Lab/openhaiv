        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                for epoch in range(args.epochs_base):
                    ...
                    # train base sess
                    tl, tl_joint, ta = base_train(self.model, trainloader, criterion, optimizer, scheduler, epoch, self.transform, args)
                    # test model with all seen class
                    tsl, tsa = test(self.model, testloader, epoch, self.transform, args, session)
                    #tl_moco, tl_moco_global, tl_moco_small,
                    ...
                    #tl_moco, tl_moco_global, tl_moco_small #, moco_loss:%.5f, moco_loss_global:%.5f, moco_loss_small:%.5f,
                    result_list.append(
                    'epoch:%03d,lr:%.4f,training_loss:%.5f,joint_loss:%.5f,training_acc:%.5f,test_loss:%.5f,test_acc:%.5f'% (epoch, lrc, tl, tl_joint, ta, tsl, tsa))
                ...
                # compute_sample_dis(trainloader, self.transform, self.model, args)
