...
def set_up_datasets(args):
    ...
    if args.dataset == 'remote':
        import dataloader.remote.remote as Dataset
        # args.base_class = 11
        # args.num_classes = 15
        # args.way = 4
        # args.shot = 20
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'remote':
        trainset = args.Dataset.remote(root=args.dataroot, train=True,index=class_index, base_sess=True,
                                       crop_transform=crop_transform, secondary_transform=secondary_transform, args=args)
        testset = args.Dataset.remote(root=args.dataroot, train=False, index=class_index, args=args)
...
def get_new_dataloader(args,session, model=None):
    crop_transform, secondary_transform = get_transform(args)
    txt_path = "/new_data/tyw/code/data/index_list/" + "remote" + "/session_" + str(session + 1) + '.txt'
    if args.dataset == 'cifar100':
    ...
    if args.dataset == 'cub200':
        trainset = args.Dataset.CUB200(root=args.dataroot, train=True, index_path=txt_path,
                                       crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'remote':
        trainset = args.Dataset.remote(root=args.dataroot, train=True, index_path=txt_path,
                                       crop_transform=crop_transform, secondary_transform=secondary_transform, args=args)
    if args.dataset == 'mini_imagenet':
    ...
    if args.dataset == 'mini_imagenet':
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'remote':
        testset = args.Dataset.remote(root=args.dataroot, train=False,
