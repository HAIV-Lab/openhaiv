...
class K_Means:
    ...
    def fit(self, X):
        # This function gets the unsupervised K_Means result of features
        random_state = check_random_state(self.random_state)
        best_inertia = None
...
def main():
    ...
    y = np.array(y)
    l_targets = np.concatenate((y[:125], y[250:375]))
    l_feats = np.concatenate((X[:125], X[250:375]))
    u_feats = np.concatenate((X[125:250], X[375:]))
    ...
    pred = pred[250:]
    label = np.concatenate((y[125:250], y[375:]))
    print('nmi', nmi_score(pred, label))
    ...
    for i in range(4):
        indices = pred == i
        plt.scatter(u_feats[indices, 0], u_feats[indices, 1], color=colors[i], s=10, label=i)

    for i in range(4):
        plt.scatter(centers[i][0], centers[i][1], s=130, marker="*", color='r')

    plt.legend()
    plt.savefig("./SemiSupKMeans.png")

    km.fit(X)
    pseudo_labels = np.array(km.labels_)
    centers = np.array(km.cluster_centers_)
    print('nmi', nmi_score(pseudo_labels, y))

    plt.figure()
    for i in range(4):
        indices = pseudo_labels == i
        plt.scatter(X[indices, 0], X[indices, 1], color=colors[i], s=10, label=i)
