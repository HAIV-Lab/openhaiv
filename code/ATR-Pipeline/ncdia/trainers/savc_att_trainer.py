...
#                data_small = None
            ...
            # ------  forward  ------- #
            joint_preds, joint_preds_att = self.net(im_cla=data_classify)  
            ...
            for i in range(m):
                ...
                agg_preds_att = agg_preds_att + joint_preds_att[i::m, :] / m

            loss = joint_loss + self.config.loss.trade_weight * att_loss  #  joint_loss + 
    ...
    def update_session_info(self, session):
        ...
        self.new_fc = nn.Parameter(
            torch.rand(self.config.CIL.way*self.num_trans, self.net.num_features, device="cpu"),
            requires_grad=True)
    ...
    def train_epoch_new(self, epoch_idx):
        ...
        with torch.enable_grad():
            for train_step in tqdm(range(1, len(train_dataiter) + 1),
                ...
                # print(joint_loss)
                # _, output_global, output_small, target_global, target_small = self.net(im_cla=data_classify)\
                #, im_q=data_query, im_k=data_key, labels=joint_labels, im_q_small=data_small, base_sess=False, last_epochs_new=(epoch==args.epochs_new-1))
                # loss_moco_global = self.supcon_loss(output_global, target_global)
                # loss_moco_small = self.supcon_loss(output_small, target_small)
                # loss_moco = self.config.CIL.alpha * loss_moco_global + self.config.CIL.beta * loss_moco_small 
