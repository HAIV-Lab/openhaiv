...
class remote(Dataset):
    ...
    def sample_test_data(self, shot=-1, ratio=0.1):
        """
        ...
        :param ratio: The fraction of data to sample from each class.
               shot: The number of data to sample from each class.
...
def get_cil_dataloader(config, session: int): #, ncd: bool):
    ...
    # # ----------- while not novel class discovery, identical to common CIL ---------- #
    # if ncd == False: 
    #     if session == 0:
    #         trainset, trainloader, testloader = get_base_dataloader(config)
    #     else:
    #         trainset, trainloader, testloader = get_new_dataloader(config, session)
    #     return trainset, trainloader, testloader
    # # ----- while novel class discovery, mix the previous session testset and current session trainset ------- #
    # elif ncd == True:
    #     if session == 0:
    #         trainset, trainloader, testloader = get_base_dataloader(config)
    #     else:  # max(0, session-1) 获得当前session之前的类别的test-set
    #         # class_old = config.dataloader.base_class + max(0, session-1) * config.dataloader.way
    #         # 之前的类别是0 -> class_old, 新的就是class_old -> class_old + config.dataloader.way
    #         trainset, testset = get_ncd_dataset(config, session) 
    #         # 需要保证可以替换trainset的transform和multi-train参数。
    #         # ncd需要改变的似乎就是trainset的label,需要改变trainloader，因为sampler变了，同时可能有data数量的减少
    #         # ncd需要model，trainset & loader
