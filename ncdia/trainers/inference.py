...
class Inference(PreTrainer):
    ...
    def eval_ood(self):
        ...
        conf = AutoOOD.inference(metrics, logits, feat, train_logits, train_feat, fc_weight, prototype)
        ...
        for i in range(len(self.hist_testset.labels)):
            if self.hist_testset.labels[i] in new_class and i in novel_indices:
                acc_num += 1
            elif self.hist_testset.labels[i] not in new_class and i not in novel_indices:
                acc_num += 1
