...
class COILHook(AlgHook):
    ...
    def solving_ot(self, trainer):
        ...
        with torch.no_grad():
            ...
            self._extract_class_means(trainer, 0, total_class + each_time_class_num)

            former_class_means = torch.tensor(self._ot_prototype_means[:total_class])
...
class COIL(BaseAlg):
    ...
    def train_step(self, trainer, data, label, attribute, imgpath):
        ...
        if session >= 1:
            ...
            self._ot_new_branch = torch.load(
                os.path.join(trainer.work_dir, branch_path)
            )
            ...
            log_pai_k = F.log_softmax(logits[:, :known_class] / T, dim=1)
    ...
    def solving_ot_to_old(self, trainer):
        ...
        total_class = self.args.CIL.base_classes + self.args.CIL.way * session
        _network = trainer.model
        self._extract_class_means_with_memory(trainer, known_class, total_class)
        former_class_means = torch.tensor(self._ot_prototype_means[:known_class])
        next_period_class_means = torch.tensor(
