...
class BEEFISO(BaseAlg):
    ...
    def _compute_loss(self, data, labels):
        ...
        loss_en = self.args["energy_weight"] * self.get_energy_loss(
            data, labels, labels
        )
    ...
    def get_energy_loss(self, inputs, targets, pseudo_targets):
        ...
        if self._cur_task == 0:
            ...
            targets = (
                targets + (self._total_classes - self._known_classes) + self._cur_task
            )
    ...
    def sample_q(self, replay_buffer, n_steps=3):
        ...
        optimizer_gen = torch.optim.SGD([embedding_k], lr=1e-2)
        for k in range(1, n_steps + 1):
            ...
            negative_energy = torch.log(
                torch.sum(torch.softmax(logits, dim=1)[:, -num_forwards:])
            )
            optimizer_gen.zero_grad()
            negative_energy.sum().backward()
            optimizer_gen.step()
            embedding_k.data += 1e-3 * torch.randn_like(embedding_k)
