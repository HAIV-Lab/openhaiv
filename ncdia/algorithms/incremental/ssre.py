...
class SSRE(BaseAlg):
    def __init__(self, trainer) -> None:
        ...
        self._protos = [] if session == 0 else np.load(os.path.join('temp/', 'protos.npy'), allow_pickle=True).tolist()
    ...
    def _compute_ssre_loss(self,inputs, targets, session):
        ...
        index = np.random.choice(range(known_class),size=self.trainer.train_loader.batch_size,replace=True)
    ...
    def train_step(self, trainer, data, label, attribute, imgpath):
        ...
        data = data.cuda()
        labels = label.cuda()
        logits, loss_clf, loss_fkd, loss_proto = self._compute_ssre_loss(data,labels,session)
        logits_ = logits[:, :self._total_classes]
