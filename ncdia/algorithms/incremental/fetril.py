...
class FeTrILHook(AlgHook):
    ...
    def before_train(self, trainer) -> None:
        ...
        if session > 0:
            ...
            trainer.algorithm._build_feature_set(
                trainer.train_loader, trainer.test_loader
            )
            trainer._train_loader = DataLoader(
                trainer.algorithm._feature_trainset,
...
class FeTrIL(BaseAlg):
    def __init__(self, trainer) -> None:
        ...
        self._means = (
            []
            if session == 0
            else np.load(os.path.join("temp/", "means.npy"), allow_pickle=True).tolist()
        )
        self._svm_accs = (
            []
            if session == 0
            else np.load(
                os.path.join("temp/", "svm_accs.npy"), allow_pickle=True
            ).tolist()
        )
    ...
    def _compute_means(self, trainer):
        with torch.no_grad():
            for class_idx in range(self._known_classes, self._total_classes):
                ...
                idx_loader = filter_dataloader_by_class(
                    trainer.train_loader, trainer.test_loader, [class_idx]
                )
    ...
    def _build_feature_set(self, train_loader, test_loader):
        ...
        for class_idx in range(0, self._known_classes):
            ...
            self.vectors_train.append(
                self.vectors_train[new_idx - self._known_classes]
                - self._means[new_idx]
                + self._means[class_idx]
            )
        ...
        for class_idx in range(0, self._total_classes):
            target_classes = [class_idx]
            filtered_loader = filter_dataloader_by_class(
                self.trainer.test_loader, self.trainer.test_loader, target_classes
            )
            vectors, _ = self._extract_vectors(filtered_loader)
    ...
    def _train_svm(self, trainer):
        ...
        train_features = (
            train_features / np.linalg.norm(train_features, axis=1)[:, None]
        )
        test_features = test_features / np.linalg.norm(test_features, axis=1)[:, None]
        svm_classifier = LinearSVC(random_state=42)
        svm_classifier.fit(train_features, train_labels)
        logging.info(
            "svm train: acc: {}".format(
                np.around(
                    svm_classifier.score(train_features, train_labels) * 100, decimals=2
                )
            )
        )
