...
class Train_LwF_New_Pipeline:
    ...
    def run(self):
        ...
        for session in range(1, self.config.CIL.sessions):
            # get dataloader
            ...
            trainer._old_net = get_network(self.config)
            trainer._old_net.freeze()
            # 用新的trainloader更新新的fc层
            trainer.train_loader = cur_train_loader
            for epoch_idx in range(1, self.config.optimizer.num_epochs + 1):
                net, train_metrics = trainer.inc_train(epoch_idx, session)
                val_metrics = evaluator.eval_acc(net, cur_val_loader, None, session,
                                                 epoch_idx)
                comm.synchronize()
                if comm.is_main_process():
