...
def get_arguments():
    ...
    parser.add_argument(
        "--config", dest="config", help="settings of CALIP in yaml format"
    )
...
def main(cfg):
    ...
    dataset = torchvision.datasets.ImageNet(
        cfg["data_root"] + cfg["dataset"], split="val", transform=preprocess
    )
    ...
    if cfg["load_cache"]:
        ...
        print(
            "No cached features and labels, start encoding image features with clip..."
        )
        total_features = []
        labels = []
        with torch.no_grad():
    ...
    # ------------------------------------------ CLIP Zero-shot ------------------------------------------
    logits = 100.0 * img_global_feat @ feat_t
    ...
    def get_logits():
        with torch.no_grad():
            ...
            for i, feat_v in enumerate(tqdm(img_spatial_feat)):
                ...
                l1 = 100.0 * img_global_feat[i] @ feat_t_a
                l2 = 100.0 * feat_v_a @ feat_t
    ...
    if cfg["search"]:
        ...
        for beta2 in beta2_list:
            for beta3 in beta3_list:
                ...
                if acc > best_acc:
                    print(
                        "New best setting, beta1: {:.4f}; beta2: {:.4f}; beta3: {:.4f}; Acc: {:.2f}".format(
                            1, beta2, beta3, acc
                        )
                    )
                    best_acc = acc
        ...
        print(
            f"Finish searching {cfg['dataset']} on backbone {cfg['backbone']}. Final Acc: {best_acc:.2f}"
        )
