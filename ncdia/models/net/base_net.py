...
class MobileNet_Base(nn.Module):
    def __init__(self, network: Configs, checkpoint: str = None, loss: str = "CrossEntropyLoss", **kwargs):
        super().__init__()
        self.args = network.cfg
        num_classes = self.args["num_classes"]
        type = self.args.get("type", "mobilenet_v3_small")  # 支持 small/large

        # 加载官方预训练权重
        if type == "mobilenet_v3_small":
            self.network = models.mobilenet_v3_small(pretrained=True)
        elif type == "mobilenet_v3_large":
            self.network = models.mobilenet_v3_large(pretrained=True)
        else:
            raise ValueError("Unsupported MobileNetV3 architecture")
