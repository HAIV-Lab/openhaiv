...
class OpenMax(nn.Module):
    ...
    def get_backbone_last_layer_out_channel(self):
        ...
        while (
            (not isinstance(last_layer, nn.Conv2d))
            and (not isinstance(last_layer, nn.Linear))
            and (not isinstance(last_layer, nn.BatchNorm2d))
        ):

            temp_layer = list(last_layer.children())[-1]
            if (
                isinstance(temp_layer, nn.Sequential)
                and len(list(temp_layer.children())) == 0
            ):
    ...
    def forward(self, x):
        ...
        feature = (
            self.embeddingLayer(feature) if hasattr(self, "embeddingLayer") else feature
        )
