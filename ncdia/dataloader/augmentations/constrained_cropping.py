...
class CustomMultiCropDataset(data.Dataset):
    def __init__(
        self, base_dataset, multi_crop, secondary_transform, return_crop_params=True
    ):
        ...
        if isinstance(secondary_transform, list):
            assert (
                len(secondary_transform)
                == self.multi_crop.N_large + self.multi_crop.N_small
            )
    ...
    def __getitem__(self, index):
        ...
        if self.return_crop_params:
            return multi_crop, multi_crop_params, target

        else:
            return multi_crop, target
...
class CustomMultiCropping(object):
    ...
    def __init__(
        ...
        if (
            (scale_large[0] > scale_large[1])
            or (scale_small[0] > scale_small[1])
            or (ratio[0] > ratio[1])
        ):
    ...
    def get_params_conditioned(self, img, scale, ratio, constraint):
        ...
        for counter in range(10):
            ...
            if 0 < w <= width and 0 < h <= height:
                ...
                if (
                    intersection >= 0.1 * target_area
                ):  # 10 percent of the small crop is part of big crop.
                    return i, j, h, w
    ...
    def __call__(self, img):
        ...
        for ii in range(self.N_large):
            ...
            multi_crop.append(
                F.resized_crop(img, i, j, h, w, self.size_large, self.interpolation)
            )
        ...
        for ii in range(self.N_small):
            ...
            multi_crop_params.append((i, j, h, w))
            multi_crop.append(
                F.resized_crop(img, i, j, h, w, self.size_small, self.interpolation)
            )
    ...
    def __repr__(self):
        format_string = self.__class__.__name__ + "(size_large={0}".format(
            self.size_large
        )
        format_string += ", scale_large={0}".format(
            tuple(round(s, 4) for s in self.scale_large)
        )
        format_string += ", size_small={0}".format(
            tuple(round(s, 4) for s in self.size_small)
        )
        format_string += ", scale_small={0}".format(
            tuple(round(s, 4) for s in self.scale_small)
        )
        format_string += ", ratio={0}".format(tuple(round(r, 4) for r in self.ratio))
        format_string += ", condition_small_crops_on_key={})".format(
            self.condition_small_crops_on_key
        )
