...
def ten_crop(
    img: Tensor, size: BroadcastingList2[int], vertical_flip: bool = False
) -> List[Tensor]:
    """DEPRECATED"""
    warnings.warn(
        "This method is deprecated and will be removed in future releases. "
...
def resize(
    ...
    if (
        isinstance(size, int) or len(size) == 1
    ...
    img = interpolate(
        img, size=[new_h, new_w], mode=interpolation, align_corners=align_corners
    )
...
def _apply_grid_transform(
    img: Tensor, grid: Tensor, mode: str, fill: Optional[List[float]]
) -> Tensor:
...
def invert(img: Tensor) -> Tensor:
    ...
    bound = torch.tensor(
        1 if img.is_floating_point() else 255, dtype=img.dtype, device=img.device
    )
    return bound - img
...
def posterize(img: Tensor, bits: int) -> Tensor:
    ...
    if img.dtype != torch.uint8:
        raise TypeError(
            "Only torch.uint8 image tensors are supported, but found {}".format(
                img.dtype
            )
        )
...
def equalize(img: Tensor) -> Tensor:
    ...
    if not (3 <= img.ndim <= 4):
        raise TypeError(
            "Input image tensor should have 3 or 4 dimensions, but found {}".format(
                img.ndim
            )
        )
    if img.dtype != torch.uint8:
