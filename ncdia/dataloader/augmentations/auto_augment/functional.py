...
import utils.auto_augment.functional_pil as F_pil
import utils.auto_augment.functional_tensor as F_t
...
def resize(
    ...
    return F_t.resize(
        img, size=size, interpolation=interpolation.value, max_size=max_size
    )
...
def posterize(img: Tensor, bits: int) -> Tensor:
    ...
    if not (0 <= bits <= 8):
        raise ValueError(
            "The number if bits should be between 0 and 8. Got {}".format(bits)
        )
